...
import math
from .ref_arch_util import EventImage_ChannelAttentionTransformerBlock
...
class EFNet(nn.Module):
    ...
    def forward_before_enc(self, x, ev, merge_before_downsample=True):
        image = x
        #EVencoder
        x1 = self.conv_01(image)
        self.ev = ev
    ...
    def forward_enc_stage(self, x, stage_idx, mask=None):
        i = stage_idx
        down = self.down_path_1[i]
        if (i+1) < self.depth:
            x1, x1_up = down(x, event_filter=self.ev[i], merge_before_downsample=self.fuse_before_downsample)
            self.encs.append(x1_up)
            if mask is not None:
                self.masks.append(F.interpolate(mask, scale_factor = 0.5**i))
        else:
            x1 = down(x, event_filter=self.ev[i], merge_before_downsample=self.fuse_before_downsample)
        return x1

    def forward_rest(self, x1, mask=None):
        decoder_feats = [x1]
        for i, up in enumerate(self.up_path_1):
            x1 = up(x1, self.skip_conv_1[i](self.encs[-i-1]))
            self.decs.append(x1)
            decoder_feats.append(x1)
        sam_feature, out_1 = self.sam12(x1, self.image)        
        #stage 2
        x2 = self.conv_02(self.image)
        x2 = self.cat12(torch.cat([x2, sam_feature], dim=1))
        blocks = []
        for i, down in enumerate(self.down_path_2):
            if (i+1) < self.depth:
                if mask is not None:
                    x2, x2_up = down(x2, self.encs[i], self.decs[-i-1], mask=self.masks[i])
                else:
                    x2, x2_up = down(x2, self.encs[i], self.decs[-i-1])
                blocks.append(x2_up)
            else:
                x2 = down(x2)
        for i, up in enumerate(self.up_path_2):
        ...
        out_2 = self.last(x2)
        out_2 = out_2 + self.image

        return [out_1, out_2, decoder_feats]
