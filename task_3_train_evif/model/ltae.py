...
class LTAE2d(nn.Module):
    ...
    # x in shape [N, T, C, H, W], T evt rec feats
    # y in shape [N, C, H, W], deblur evt feat
    # batch_positions [N, T]
    def forward(self, x, y, batch_positions=None, pad_mask=None, return_comp=False):
        ...
        # normalize along T [bs*h*w, T, d]
        out = self.in_norm(out.permute(0, 2, 1)).permute(0, 2, 1)
        if batch_positions is None:
            batch_positions = torch.zeros((sz_b, seq_len))
            for i in range(seq_len):
                batch_positions[:,seq_len-i-1] = i * 30.0 # set position to ...,60,30,0
            batch_positions = batch_positions.to(x.device)
        # [bs*h*w, d]
        out_y = y.permute(0, 3, 4, 1, 2).contiguous().view(sz_b_y * h_y * w_y, 1, d)
        ...
        # input: [bs*h*w, T, d]
        out, attn = self.attention_heads(out, out_y, pad_mask=pad_mask)
...
class MultiHeadCrossAttention(nn.Module):
    ...
    # will use 'v' to compute 'k', and use 'x' to get 'q'
    # 'v' has a shape of : [bs*h*w, T, d]
    # 'x' has a shape of : [bs*h*w, 1, d]
    def forward(self, v, x, pad_mask=None, return_comp=False):
        d_k, d_in, n_head = self.d_k, self.d_in, self.n_head
        sz_b, seq_len, _ = v.size()

        q = self.fc1_q(x).view(sz_b, 1, n_head, d_k)
        q = q.permute(2, 0, 1, 3).contiguous().view(-1, d_k)  # (n*b) x d_k
