...
def add_hot_pixels_to_voxels(item, hot_pixel_std=1.0, max_hot_pixel_fraction=0.001):
    hot_pixel_fraction = random.uniform(0, max_hot_pixel_fraction)
    voxel = item['blurry_events']
    num_hot_pixels = int(hot_pixel_fraction * voxel.shape[-1] * voxel.shape[-2])
    x = torch.randint(0, voxel.shape[-1], (num_hot_pixels,))
    y = torch.randint(0, voxel.shape[-2], (num_hot_pixels,))
    val = torch.randn(num_hot_pixels, dtype=voxel.dtype, device=voxel.device)
    val *= hot_pixel_std
    for i in range(num_hot_pixels):
        item['blurry_events'][..., :, y[i], x[i]] += val[i]

    for j in range(len(item['history_events'])):
        for i in range(num_hot_pixels):
            item['history_events'][j][..., :, y[i], x[i]] += val[i]
    for j in range(len(item['prev_history_events'])):
        for i in range(num_hot_pixels):
            item['prev_history_events'][j][..., :, y[i], x[i]] += val[i]
    return item
