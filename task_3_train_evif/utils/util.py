...
def get_height_width(data_loader):
    for d in data_loader:
        return d['blurry_events'].shape[-2:]  # d['events'] is a ... x H x W voxel grid 
...
def torch2cv2_rgb(image):
    """convert torch tensor to format compatible with cv2.imwrite"""
    image = torch.squeeze(image)  # 3 x H x W
    image = image.permute(1,2,0).contiguous()  # H x W x 3
    image = image.cpu().numpy() 
    image = image[:,:,::-1]
    image = np.clip(image, 0, 1)
    return (image * 255).astype(np.uint8)
