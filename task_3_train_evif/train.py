...
def load_ckpt_densefuse(model, ckpt_path):
    state_dict = torch.load(ckpt_path)
    #config = ckpt['config']
    #write_json(config.config, ckpt_path+'config.json')
    try:
...
def main(config):
    ...
    try:
       model.fusion_enc_vis = load_ckpt_densefuse(model.fusion_enc_vis, config['resume_ckpt_densefuse'])
       print('densefuse encoder vis load success')
    except:
       traceback.print_exc()
       pass

    try:
       model.fusion_enc_inf = load_ckpt_densefuse(model.fusion_enc_inf, config['resume_ckpt_densefuse'])
       print('densefuse encoder inf load success')
    except:
       traceback.print_exc()
       pass

    try:
       model.fusion_dec = load_ckpt_densefuse(model.fusion_dec, config['resume_ckpt_densefuse'])
       print('densefuse decoder load success')
    except:
       traceback.print_exc()
       pass
    ...
    try:
       ...
       try:
          model = load_ckpt(model, config['resume_ckpt_full'], strict=False)
          print('full model not strict load success')
       except:
          traceback.print_exc()
          pass
    ...
    trainable_params = []
    trainable_params += list(filter(lambda p: p.requires_grad, model.fusion_enc_vis.parameters()))
    trainable_params += list(filter(lambda p: p.requires_grad, model.fusion_enc_inf.parameters()))
    trainable_params += list(filter(lambda p: p.requires_grad, model.fusion_dec.parameters()))
    trainable_params += list(filter(lambda p: p.requires_grad, model.align_dws_conv_inf.parameters()))
    trainable_params += list(filter(lambda p: p.requires_grad, model.align_dws_conv_vis.parameters()))
    trainable_params += list(filter(lambda p: p.requires_grad, model.fusion_transformers.parameters()))
    trainable_params += list(filter(lambda p: p.requires_grad, model.mi_minimizers.parameters()))
    trainable_params += list(filter(lambda p: p.requires_grad, model.mi_maximizers_inf.parameters()))
    trainable_params += list(filter(lambda p: p.requires_grad, model.mi_maximizers_vis.parameters()))
