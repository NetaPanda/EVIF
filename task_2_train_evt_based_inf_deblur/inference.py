...
def main(args, model):
    ...
    data_loader = InferenceDataLoader(args.events_file_path, args.deblur_file_path, dataset_kwargs=dataset_kwargs, ltype=args.loader_type)
    ...
    for i, item in enumerate(tqdm(data_loader)):
        model.reset_states()
        blur           = item['blurry_frame'].float().to(device)
        blurry_events  = item['blurry_events'].float().to(device)
        history_events = item['history_events'].float().to(device)
        image          = item['sharp_frame'].float().to(device)
        with CudaTimer('Inference'):
            output = model(blur, blurry_events, history_events)
        # save sample images, or do something with output here
        psnr  = -1 * ploss(output['image'], image)
        print('psnr: ', psnr)
        mean_psnr += psnr
        if args.color:
            pred_image = output['image']
        else:
            pred_image = crop.crop(output['image'])
            pred_image = torch2cv2(pred_image)
            gt_image = crop.crop(image)
            gt_image = torch2cv2(gt_image)
            blur_image = crop.crop(blur)
            blur_image = torch2cv2(blur_image)
        fname = 'frame_{:010d}_blur.png'.format(i)
        cv2.imwrite(join(args.output_folder, fname), blur_image)
        fname = 'frame_{:010d}_gt.png'.format(i)
        cv2.imwrite(join(args.output_folder, fname), gt_image)
        fname = 'frame_{:010d}_pred.png'.format(i)
        cv2.imwrite(join(args.output_folder, fname), pred_image)
        count += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--deblur_file_path', required=True, type=str,
                        help='path to deblur data (HDF5)')
