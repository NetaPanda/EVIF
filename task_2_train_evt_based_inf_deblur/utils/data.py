...
def concatenate_datasets(data_file, data_file_deblur, dataset_type, dataset_kwargs={}):
    """
    ...
    data_paths_deblur = pd.read_csv(data_file_deblur, header=None).values.flatten().tolist()
    dataset_list = []
    print('Concatenating {} datasets'.format(dataset_type))
    for data_path, data_path_deblur in tqdm(zip(data_paths, data_paths_deblur)):
        dataset_list.append(dataset_type(data_path,data_path_deblur,**dataset_kwargs))
