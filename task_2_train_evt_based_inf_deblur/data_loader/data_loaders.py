...
class InferenceDataLoader(DataLoader):
    ...
    def __init__(self, data_file, data_file_deblur, num_workers=1, pin_memory=True, dataset_kwargs=None, ltype="H5"):
        if dataset_kwargs is None:
            dataset_kwargs = {}
        if ltype == "H5":
            dataset = DynamicH5Dataset(data_file, data_file_deblur, **dataset_kwargs)
        elif ltype == "MMP":
...
class HDF5DataLoader(DataLoader):
    """
    """
    def __init__(self, data_file, data_file_deblur, batch_size, shuffle=True, num_workers=1,
                 pin_memory=True, dataset_kwargs={}):
        dataset = concatenate_datasets(data_file, data_file_deblur, DynamicH5Dataset, dataset_kwargs)
