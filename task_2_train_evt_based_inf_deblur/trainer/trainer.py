...
class Trainer(BaseTrainer):
    ...
    def __init__(self, model, loss_ftns, optimizer, config, data_loader,
        ...
        for data_source in data_sources:
            ...
            for l in self.loss_ftns:
                ...
                mt_keys.append(f'{l.__class__.__name__}_sam/{data_source}')
    ...
    def forward_sequence(self, item, all_losses=False):
        ...
        for loss_ftn in self.loss_ftns:
            ...
            if loss_name == 'perceptual_loss':
                losses[loss_name].append(loss_ftn(pred['image'], gt_norm, normalize=True))
                losses[loss_name+'_sam'].append(loss_ftn(pred['image_sam'], gt_norm))
            if loss_name == 'l2_loss':
                losses[loss_name].append(loss_ftn(pred['image'], gt_norm))
                losses[loss_name+'_sam'].append(loss_ftn(pred['image_sam'], gt_norm))
            if loss_name == 'psnr_loss':
                losses[loss_name].append(loss_ftn(pred['image'], gt_norm))
                losses[loss_name+'_sam'].append(loss_ftn(pred['image_sam'], gt_norm))
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, item in enumerate(self.data_loader):
            ...
            losses = self.forward_sequence(item)
            loss = losses['loss']
