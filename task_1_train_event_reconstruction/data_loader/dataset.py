...
class BaseVoxelDataset(Dataset):
    ...
    For each index, returns a dict containing:
        ...
        * flow is a 2 x H x W tensor containing the flow (displacement) from
          the current frame to the last frame ----> this comment was wrong  because the original code use forward flow, but now it is right because I use backward flow
    ...
    def __getitem__(self, index, seed=None):
        ...
        if self.voxel_method['method'] == 'between_frames':
            ...
            if self.has_flow:
                ...
                # flow = flow * dt # gmy: why? isnt flow already displacement?
...
class DynamicH5Dataset(BaseVoxelDataset):
    ...
    def get_frame(self, index):
        frame = self.h5_file['images']['image{:09d}'.format(index)][:]
